...
const SupportTicketPage = () => {
  ...
  const [status, setStatus] = useState('');
	...
	const fetchTransactions = useCallback(async () => {
		try {
			const response = await axios.get('/v1/officer/support', {
				headers: {
    ...
    const newData = transactionLogs.filter((record) => {
			const matchesSearch =
				searchName === '' ||(record.student.name + ' ( ' + record.student.studentID + ' ) ').toLowerCase()
						.toLowerCase()
						.includes(searchName.toLowerCase());

        const matchesID = searchID === '' || record.transactionID.toLowerCase().includes(searchID.toLowerCase());
        const printDate = new Date(record.createdAt);
        const matchesStartDate = !startDate || printDate >= startDate;
        const matchesEndDate = !endDate || printDate <= endDate;
        const matchesErrorName = errorName === '' || record.description.toLowerCase().includes(errorName.toLowerCase());
        const matchesStatus = status === '' || record.status.toLowerCase().includes(status.toLowerCase());

        return matchesSearch && matchesID && matchesStartDate && matchesEndDate && matchesErrorName && matchesStatus;
		});
...
<select
								...
								value={status}
                defaultValue={''}
								onChange={(st) => setStatus(st.target.value)}
							>
            ...
            <div className='mb-8 rounded-lg bg-white p-4 shadow'>
                ...
                <th className='px-4 py-2 text-left'>
                  ...
                  <td className='px-4 py-2'>{item.student.name + ' ( ' + item.student.studentID + ' ) '}</td>
									...
									<td className='px-4 py-2'>
										{(item.status === 'closed')? <span className='text-green-600'> Đã xử lý
										</span> : <span className='text-red-600'>
                      Đang chờ xử lý
										</span>}
									</td>
                  <td className='px-4 py-2'>{new Date(item.createdAt).toLocaleDateString()}</td>
                  {(item.closedAt) && <td className='px-4 py-2'>{new Date(item.closedAt).toLocaleDateString()}</td>}
								</tr>
							))}
						</tbody>
					</table>
